...
class DiaryTransformerApp(tk.Tk):
    def __init__(self):
        ...
        # System prompt entry
        tk.Label(self, text="시스템 지시어:").pack(pady=(10, 0))
        self.sys_entry = tk.Entry(self, width=70)
        self.sys_entry.insert(0, "You are a friendly diary assistant.")
    ...
    def call_gemini_for_diary(self, text: str) -> str:
        ...
        prompt = f'다음 텍스트를 5세에서 13세 사이의 아이가 그림일기에 쓸 법한 3~5 문장의 짧은 글로 바꿔줘. 친근하고 쉬운 단어를 사용해줘:\n\n"{text}"'
    ...
    def call_gemini_generate(self, text: str, sys_prompt: str):
        # Generate both text and image using Gemini API
    ...
    def generate_image(self):
        ...
        if not original_content:
            ...
            return
        try:
            text, image = self.call_gemini_generate(original_content, sys_prompt)
